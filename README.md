...
## 시연영상
유튜브 링크 : https://youtu.be/XDT32ptfMLA
...
<details>
<summary>롱노트 콤보 판정의 오류</summary>

### 현상  
같은 곡을 시연할 때 마다 같은 롱노트로부터 얻어지는 콤보의 수가 간혹 달라지는 현상 발생

### 원인  
- 롱노트를 누르는 동안의 콤보 획득 로직을 프레임에 의존하는 Update 함수에서 계산한 것이 원인
- 고정적인 콤보 수치가 아닌 프레임으로 콤보를 실시간으로 계산하게 되면 당시 프레임 성능마다 얻어지는 콤보가 달라졌던 것

### 해결 방식  
- 롱노트를 생성할 시 채보 데이터의 롱노트 지속 시간(Duration)과 간격(Combo Interval)을 설정해
  사전에 먼저 해당 롱노트를 통해 얻어지는 고정적인 콤보 수 (TargetCombo)를 계산하여 롱노트가 딱 이 콤보 수 만큼만 판정되도록 수정하여 해결
